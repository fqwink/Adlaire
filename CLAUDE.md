...
全プロジェクトに例外なく適用する。Adlaire Static CMS は **Ver.3.1 より適用**。

| バージョン | 種別 | 規定 |
|-----------|------|------|
| **Ver.X.0** | 破壊的変更 | Major バージョン規則に準拠。後方互換性のない変更を含む。 |
| **Ver.X.1** | 破壊的変更 または 破壊的改良 | 破壊的変更・改良がない場合は通常の奇数バージョン扱い。 |
| **偶数（.2/.4/.6/.8）** | バグ修正 | 50件以上〜100件の精査・実装。Ver.X.1 を除く。 |
| **奇数（.3/.5/.7）** | 新機能 + 機能改良 | 新機能：計15件 / 機能改良：計20〜30件 |
| **Ver.X.9** | バグゼロ達成 | 残存バグをゼロにする最終バグ修正バージョン。奇数の例外。 |

- Ver.X.0 から Ver.X.9 で 1 サイクル（10バージョン）を構成する。
