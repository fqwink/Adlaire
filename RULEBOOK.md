...
**format**: `blocks`（ブロックエディタJSON、デフォルト）, `markdown`（Markdownテキスト）
...
| メソッド | URL | レスポンス |
|---------|-----|-----------|
| `GET` | `?api=sitemap` | sitemap.xml（公開ページのみ） |

### 5.5 エクスポート・インポート API

| メソッド | URL | レスポンス |
|---------|-----|-----------|
| `GET` | `?api=export` | JSON ファイルダウンロード（config + 全ページ） |
...
- ブロック間の `+` ボタンでツールボックス表示（ブロック追加、i18n 対応ラベル）
- ブロック左側: `+`（追加）`▲▼`（移動）`×`（削除）ボタン
- インラインツールバー: テキスト選択時に太字(B)/斜体(I)/リンク(🔗)
- Enter: 新規段落ブロック追加（リスト項目では新 li 追加）
- Backspace: 空ブロック削除、前ブロックにフォーカス移動
- 保存インジケーター: ✓（保存成功）/ ✗（エラー）表示
