...
| # | カテゴリ | 改良点 | 状態 |
|---|---------|--------|:----:|
| 1 | バグ | ステータス保存ロジック修正（二重 API 呼び出し解消） | 計画 |
| 2 | バグ | 「View Site」リンクの言語判定を `$app->language` ベースに変更 | 計画 |
| 3 | バグ | `renderAdminNewPage` の不要な `csrf_token()` 呼び出し削除 | 計画 |
| 4 | バグ | 静的生成のメニューリンクを正しい相対パスに修正 | 計画 |
| 5 | バグ | `renderBlocksToHtml` の heading タグ計算を簡素化 | 計画 |
| 6 | バグ | 管理 UI settings の CSRF ワンタイム問題を修正（トークン更新） | 計画 |
| 7 | セキュリティ | ブロックデータの data 属性出力を安全なエンコーディングに変更 | 計画 |
| 8 | 管理UI | ダッシュボードにページ削除ボタン追加 | 計画 |
| 9 | 管理UI | ページ一覧のソート機能（更新日順） | 計画 |
| 10 | 管理UI | インポート UI 追加（ファイルアップロード） | 計画 |
| 11 | 管理UI | 設定フォームの保存フィードバック表示 | 計画 |
| 12 | 管理UI | admin CSS を外部ファイルに分離 | 計画 |
| 13 | コード品質 | `handleEdit` と `apiPageSave` の重複ロジック統一 | 計画 |
| 14 | コード品質 | admin.php から不要な `settings()` メソッド削除 | 計画 |
| 15 | コード品質 | `listPages()` にページインデックスキャッシュ導入 | 計画 |

### 9.2 Ver.1.8 — 機能拡張・フロントエンド強化

| # | カテゴリ | 改良点 | 状態 |
|---|---------|--------|:----:|
| 16 | 新機能 | ページ並び順のドラッグ&ドロップ管理 | 計画 |
| 17 | 新機能 | 管理 UI ダークモード対応 | 計画 |
| 18 | 新機能 | ページプレビュー機能（下書きを公開レイアウトで確認） | 計画 |
| 19 | 新機能 | サイドバー（subside）もブロックエディタで編集 | 計画 |
| 20 | 新機能 | 静的生成で Markdown をサーバーサイド HTML 変換 | 計画 |
| 21 | エディタ | ブロックのコピー＆ペースト（Ctrl+C/V） | 計画 |
| 22 | エディタ | Undo/Redo 機能（Ctrl+Z/Y） | 計画 |
| 23 | エディタ | ブロックのドラッグ&ドロップ並び替え | 計画 |
| 24 | エディタ | heading ブロックのレベル変更 UI（h1/h2/h3 切替） | 計画 |
| 25 | エディタ | list ブロックの順序/非順序切替 UI | 計画 |
| 26 | フロント | 公開ページから管理用 JS を除外 | 計画 |
| 27 | フロント | 静的サイト用軽量 CSS（エディタ CSS 除外） | 計画 |
| 28 | データ | エクスポートにリビジョンを含むオプション | 計画 |
| 29 | i18n | 管理 UI ラベル（Dashboard, Pages, Edit 等）の翻訳対応 | 計画 |
| 30 | ドキュメント | RULEBOOK を Ver.1.7/1.8 の実装結果で更新 | 計画 |
