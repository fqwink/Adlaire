...
| Ver.2.0 | 2026-04-06 | Phase 6〜9 詳細仕様を追記（新機能・改良機能の追加）: Phase 6（ロールバック機能・ログ SSE ストリーミング・Worker ヘルスチェック & 自動再起動・プライベートリポジトリ対応）、Phase 7（ビルドステップ・ログ永続化・環境変数暗号化・デプロイ通知 Webhook）、Phase 8（edge 自動回復）、Phase 9（管理ダッシュボード Web UI）を定義。§5 保留項目から管理ダッシュボード・監査ログを削除し計画済みに移行。§8 開発フェーズ表に Phase 6〜9 を追加。文書バージョンを Ver.1.0 から Ver.2.0 に改訂。 |
| Ver.4.0 | 2026-04-06 | Phase 14 詳細仕様を追記（VPS 運用基盤）: P14.1（インストールスクリプト・冪等セットアップ・自動シークレット生成）、P14.2（systemd サービス化・セキュリティ強化オプション）、P14.3（セルフアップデート・ゼロダウンタイム切替・自動ロールバック・VERSION ファイル）、P14.4（GitHub Actions CI/CD テンプレート・Webhook ワークフロー）、P14.5（バックアップ・リストア CLI）、P14.6（マルチVPS プロビジョニングスクリプト）を定義。§8 開発フェーズ表に Phase 14 を追加。文書バージョンを Ver.3.0 から Ver.4.0 に改訂。 |
| Ver.3.0 | 2026-04-06 | Phase 10〜13 詳細仕様を追記（新機能・改良機能の追加）: Phase 10（監査ログ・デプロイパイプライン並列化・モノレポ対応）、Phase 11（静的サイトホスティング・メトリクス収集）、Phase 12（Blue-Green デプロイ・カナリアデプロイ）、Phase 13（デプロイスケジューリング・デプロイプレビュー URL）を定義。§8 開発フェーズ表に Phase 10〜13 を追加。文書バージョンを Ver.2.0 から Ver.3.0 に改訂。 |
