...
| **デュアルデプロイ対応** | Deno Deploy・Adlaire Deploy の両プラットフォームに対応する |
...
| デプロイ対象 | Deno Deploy 専用 | **Deno Deploy + Adlaire Deploy** |
...
| `adlaire build` | 本番ビルドを生成（`_build/` に出力） |
...
| 11 | **本番ビルド最適化** — `adlaire build` / コード分割 / 静的生成 | 計画 |
