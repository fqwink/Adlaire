...
> **注**: TypeScript をコンパイルした JavaScript 出力（§10.3）は Deno ランタイム専用ではない。ブラウザ（SPA 出力）・JavaScript 実行環境が利用可能な共用サーバの両方を対象とする。
...
- **Adlaire Static CMS 管理画面 TypeScript SPA**（採用対象。§10.3 コンパイル済み JS ブラウザ出力により適用）
...
|------|------|
| UI レンダリング層 | **計画なし（対象外）** |
| Islands アーキテクチャ | **計画なし（対象外）** |
| 共用ホスティング — コンパイル済み JS によるアプローチ | §10.3 に定義済み（ブラウザ SPA 出力 / 共用サーバ出力の 2 モード） |
| 共用ホスティング — Apache + PHP アプローチの採否 | **未決定** |
...
| ビルド出力 | TypeScript ソース / TypeScript からコンパイルした JavaScript（ブラウザ・共用サーバ・Deno） |
| デプロイターゲット | Deno Deploy / Adlaire Deploy（Deno ランタイム）/ コンパイル済み JS（ブラウザ SPA・共用サーバを含む） |
...
| モード | オプション | 出力先 | 動作環境 |
|--------|-----------|--------|---------|
| **ブラウザ SPA** | `--target=js --mode=spa` | `build/js/` | ブラウザ（HTTP サーバー層を含まない SPA バンドル） |
| **共用サーバ** | `--target=js --mode=server` | `build/js/` | JavaScript 実行環境が利用可能な共用サーバ（Deno ランタイム不要） |

### ブラウザ SPA 出力

- HTTP サーバー層（`Deno.serve`）を含まず、ファイルベースルーティングをクライアントサイドルーティングとして出力する。
- **Adlaire Static CMS 管理画面 TypeScript SPA のビルドに適用する**（PHP バックエンドはそのまま維持し、SPA 部分のみ Adlaire Framework を採用）。
- 生成された JavaScript は `<script>` タグで読み込む形式（esbuild IIFE バンドル互換）で出力する。
...
| `"js"` | TypeScript → JavaScript コンパイル出力（`build/js/`）。`--mode=spa`（ブラウザ SPA）または `--mode=server`（共用サーバ）を指定する |
...
| `adlaire build --target=js --mode=spa` | TypeScript → ブラウザ SPA 用 JavaScript 出力（`build/js/`。HTTP サーバー層なし） |
| `adlaire build --target=js --mode=server` | TypeScript → 共用サーバ向け JavaScript 出力（`build/js/`） |
