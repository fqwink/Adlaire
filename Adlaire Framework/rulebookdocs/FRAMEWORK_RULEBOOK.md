...
Deno をランタイムとし、Deno Deploy および Adlaire Deploy の双方への展開に対応する。
また、**TypeScript → PHP トランスパイル機能**を付随し、共用ホスティング（Apache + PHP）への展開も実現する。
...
| **マルチデプロイ対応** | Deno Deploy・Adlaire Deploy・共用ホスティング（PHP）の 3 プラットフォームに対応する |
| **TypeScript 単一ソース** | 全ソースコードを TypeScript で開発する。PHP は TypeScript からトランスパイルして生成する |
...
| 設定ファイル | `deno.json` |
| PHP トランスパイル | TypeScript → PHP 自動変換（`adlaire build --target=php`）|
...
> **計画段階の仕様**。Adlaire Framework の開発計画として記録する。
> 実装は Adlaire Framework の開発フェーズに従い着手する（§13 参照）。
...
> **開発ステータス: 計画段階**
> 実装は Adlaire Framework の採用決定時に再度策定・着手する。それまで開発は中断予定。
