...
> **Adlaire Framework は Deno ランタイムで動作する。Deno Deploy および Adlaire Deploy の双方に対応する。**
...
| **デプロイターゲット** | Deno Deploy と Adlaire Deploy の双方で動作することを保証する |
...
| ターゲット | 方式 |
|-----------|------|
| **Deno Deploy** | エントリポイント（`main.ts`）を指定してデプロイ。エッジランタイムと完全互換 |
...
> 絶対原則§1.4「Deno ランタイム」に基づき、Deno Deploy と Adlaire Deploy の双方への対応を保証する。
