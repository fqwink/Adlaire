...
| 開発ランタイム | Deno 2.x 以上 |
| ビルド出力 | TypeScript（ソース）/ TypeScript からコンパイルした JavaScript（Deno ランタイム専用） |
| デプロイターゲット | Deno Deploy / Adlaire Deploy（Deno ランタイム）/ コンパイル済み JavaScript（Deno ランタイム専用） |
...
- 生成された JavaScript は **Deno ランタイム上のみで動作する**。Node.js・Bun 等の他ランタイムへの対応は計画しない。
