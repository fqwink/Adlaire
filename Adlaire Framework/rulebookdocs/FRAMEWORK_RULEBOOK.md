...
| 共用ホスティング — コンパイル済み JS によるアプローチ | §10.3 に定義済み（JavaScript 実行環境が利用可能な共用サーバを対象） |
...
| 開発ランタイム | Deno 2.x 以上 |
| ビルド出力 | TypeScript（ソース）/ TypeScript からコンパイルした JavaScript |
| デプロイターゲット | Deno Deploy / Adlaire Deploy（Deno ランタイム）/ コンパイル済み JavaScript（共用サーバを含む） |
...
## 10.3 コンパイル済み JavaScript デプロイ

TypeScript ソースを JavaScript にコンパイルした出力も有効なデプロイ成果物とする。

- `adlaire build --target=js` を実行すると、TypeScript から JavaScript にコンパイルされた出力を `build/js/` に生成する。
- 生成された JavaScript は Deno ランタイムおよび JavaScript 実行環境が利用可能な**共用サーバでも動作するケースを可能にする**。
- コンパイル済み JavaScript は TypeScript ソースと同等の動作を保証する。
- TypeScript 型情報はコンパイル時に除去されるが、§1.2 絶対原則「型安全」の検証はコンパイル前に行われる（ビルドエラーとして報告）。

### 共用サーバ動作の前提条件

| 条件 | 内容 |
|------|------|
| JavaScript 実行環境 | 共用サーバ上で JavaScript が実行できること（例: Node.js 対応の共用ホスティング） |
| 対象外 | Node.js 等の他ランタイム向け汎用対応は計画しない。共用サーバ上で動作するケースを可能にするものであり、他ランタイムの完全サポートとは異なる |
...
| `"js"` | TypeScript → JavaScript コンパイル出力（`build/js/`） |
...
| `adlaire build --target=js` | TypeScript → JavaScript コンパイル出力（`build/js/`） |
