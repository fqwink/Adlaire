...
> **絶対原則①: 共用ホスティング対応**
> Adlaire Framework は Apache + PHP 8.3+ の共用ホスティング環境で動作することを**絶対原則**とする。
> いかなる設計変更・機能追加においても、この原則を損なってはならない。

> **絶対原則②: PHP 自己完結生成**
> TypeScript → PHP トランスパイル後の成果物は、**PHP ネイティブコードのみで自己完結**しなければならない。
> 外部ライブラリ・フレームワーク・ランタイム依存を一切持たない純粋な PHP を生成する。
> `php-runtime/` 等の補助ライブラリへの依存を禁止する。
...
| **共用ホスティング対応（絶対原則①）** | Apache + PHP 8.3+ の共用ホスティング環境での動作を保証する。この原則はいかなる設計変更においても維持しなければならない |
| **PHP 自己完結生成（絶対原則②）** | トランスパイル後の PHP は PHP ネイティブコードのみで自己完結する。外部ライブラリ・フレームワーク・補助ランタイムへの依存を一切持たない |
...
- 変換後 PHP は **外部依存ゼロ**。`_runtime/`・`php-runtime/` 等の補助ライブラリを生成・参照しない
- Framework API 呼び出しは **PHP ネイティブコードにインライン展開**する
- 変換は **4 レイヤーパイプライン**で処理する（テキスト変換 + 限定 AST）
- 変換対象を **Framework API 範囲の TypeScript** に限定する
...
TypeScript ソース
      ...
      ↓ Layer 3: Framework API インライン展開（核心）
      ↓ Layer 4: 構文正規化
自己完結 PHP
```

各レイヤーは独立して単体テスト可能。Layer 3 のみ AST 解析を使用し、他はテキスト変換。
...
- 変換対象は Framework API 範囲内の TypeScript に限定する（任意 TypeScript の完全変換は目標としない）
