...
const _mdEscAttr = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
function markdownToHtml(md) {
    ...
    _mdHeading3.lastIndex = 0;
    html = html.replace(_mdHeading3, '<h3>$1</h3>');
    _mdHeading2.lastIndex = 0;
    html = html.replace(_mdHeading2, '<h2>$1</h2>');
    _mdHeading1.lastIndex = 0;
    html = html.replace(_mdHeading1, '<h1>$1</h1>');
    // Horizontal rule
    _mdHr.lastIndex = 0;
    html = html.replace(_mdHr, '<hr>');
    // Bold and italic
    // Ver.2.9 TS#62: 事前コンパイル済み正規表現使用
    _mdBoldItalic.lastIndex = 0;
    html = html.replace(_mdBoldItalic, '<strong><em>$1</em></strong>');
    _mdBold.lastIndex = 0;
    html = html.replace(_mdBold, '<strong>$1</strong>');
    _mdItalic.lastIndex = 0;
    html = html.replace(_mdItalic, '<em>$1</em>');
    // Images ![alt](url) — must come before links
    // Ver.2.9 #34/#63: 正規表現改善 + 事前コンパイル化
    _mdImage.lastIndex = 0;
    html = html.replace(_mdImage, (_m, alt, url, title) => {
    ...
    // Ver.2.9 #34/#63: 正規表現改善 + 事前コンパイル化
    _mdLink.lastIndex = 0;
    html = html.replace(_mdLink, (_m, text, url, title) => {
        if (_mdDangerousProto.test(url))
