...
- **メインマスター管理者アカウント**は初回導入時に安全に作成されなければならない
- メインマスター管理者のログインID はセットアップ時にユーザーが入力しなければならない
- 初期ログインID・初期パスワードの固定値を採用してはならない
- 弱いパスワードを無条件で許可してはならない
- ユーザーデータは `data/system/users.json` に保存しなければならない（`config.json` にパスワード保存禁止）
- `users.json` のファイル権限は 0600 に設定しなければならない
- setup report を出力しなければならない

### 9.2 禁止事項

- 平文パスワード保存
- `config.json` へのパスワード保存
...
- マスター管理者認証（メインマスター: ID+パスワード、サブマスター: ID+パスワード+トークン）
- `data/system/users.json` の排他ロック + アトミック書き込み + ファイル権限 0600
- サブマスター認証情報の平文はサーバー側に保存しない（ハッシュのみ）
- CSRF 保護
- lock 制御
- ログの秘匿情報抑制
- manifest 不在時停止
...
| `rulebookdocs/RELEASE_PLAN_RULEBOOK.md`（統合リポジトリルート・Part I） | リリース計画・リリース履歴 |
