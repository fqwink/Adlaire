...
| `rulebookdocs/RELEASE_PLAN_RULEBOOK.md`（統合リポジトリルート） | 分類ベース | リリース計画・リリース履歴（全プロジェクト統合管理） |
...
| ユーザーデータは `data/system/users.json` に保存（`config.json` にパスワード保存禁止） | MUST |
| `users.json` のファイル権限は 0600（owner のみ読み書き可） | MUST |
