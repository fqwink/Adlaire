...
| Ver.1.6 | 2026-04-13 | **Portable Text コンテンツ形式仕様策定**。§2.2 ページデータを Portable Text（`body` フィールド）形式に更新（旧 `content`/`format`/`blocks` フィールドを Ver.3.5 廃止予定として明記）。§2.9 Portable Text スキーマ仕様を新設（PTBlock・PTSpan・PTMarkDef・カスタム型・マイグレーション仕様）。§3.1 `migrate()` 説明更新（旧 blocks/markdown → PT 自動変換）・`writePage()` シグネチャ変更（`body` 受け取り）。§3.4 `renderPortableTextToHtml()` を新設・`renderMarkdownToHtml()`/`renderBlocksToHtml()` を Ver.3.5 廃止予定として明記。§4.1 REST API GET/POST パラメータを `body` 形式に更新。§5.1 `markdown.ts` Ver.3.5 廃止予定注記追加。§5.3.2 ブロック型を Portable Text 形式に全面改訂。§7.1 コンテンツ管理機能リストに PT 導入・旧形式廃止予定を追記。 |
...
| Ver.1.4 | 2026-04-13 | **Ver.3.2→3.3 バージョン参照更新**（リリースカレンダー規則適用によるブログ機能拡充の Ver.3.2→3.3 移動）。§3.2 生成物一覧の category/tag/日付アーカイブ記述を「Ver.3.2 以降」→「Ver.3.3 以降」に変更。§7 `generateArchivePages()` 関数説明を「Ver.3.2 以降」→「Ver.3.3 以降」に変更。§9.3 セクション見出しを「アーカイブページ生成（Ver.3.2）」→「アーカイブページ生成（Ver.3.3）」に変更。§9.4 セクション見出しを「前後ナビ（Ver.3.2）」→「前後ナビ（Ver.3.3）」に変更。§10.1 テーマファイル表の `blog-archive.php` 説明を「アーカイブテンプレート（Ver.3.2）」→「アーカイブテンプレート（Ver.3.3）」に変更。 |
| Ver.1.5 | 2026-04-13 | **Portable Text 対応・コンテンツ変換仕様更新**。§5 コンテンツ変換を Portable Text（Ver.3.1 以降）対応に全面改訂: `renderPortableTextToHtml()` を主関数として追加、旧 `renderBlocksToHtml()`/`renderMarkdownToHtml()` を Ver.3.5 廃止予定として明記。§5.1 カスタム型レンダリング（table/accordion）を PT 形式に更新。§7 `handleApiGenerate()` / `generatePageHtml()` 説明を `renderPortableTextToHtml()` 参照に更新。§9.2.5 excerpt 生成ルールを PT 形式に更新（style: "normal" ブロックから children テキスト連結）。§11.5 title 抽出規則を PT 形式に更新（heading style ブロック優先）。`$posts` 配列から `format` フィールドを削除。 |
...
| Ver.3.1 | 2026-04-13 | **Ver.3.5/3.7 仕様策定完了注記・BaaS先送り明記**（統合ファイルでの改訂）。§5.6 Ver.3.5「暫定」解除・仕様策定完了注記追加（EDITOR_RULEBOOK.md §14 / API_RULEBOOK.md §2.8, §4.9, §5.3.2 参照）。§5.8 Ver.3.7「暫定」解除・BaaS 行削除・先送り（導入時期未定）明記・仕様策定完了注記追加。Part IV BaaS リリース計画に BaaS 連携先送り決定（2026-04-13）を明記。 |
| Ver.3.2 | 2026-04-13 | **Ver.4.0 系リリース計画（§6）を新設**（統合ファイルでの改訂）。§6.0 前提・方針（破壊的変更根拠・先送り項目・不採用確定項目・再検討予定項目）、§6.1-6.8 Ver.4.0-4.7 各バージョン計画（テーマシステム刷新・管理機能強化・静的配信強化・エディタ/UI/生成/ブログ/セキュリティ/Lifecycle/国際化/データ管理の追加機能・機能改良を含む）を策定。§7-§9 へリナンバー（旧§6-§8）。§8 バージョン未配分項目を整理（A1-A6 全件配分済みとして削除、B1/B3/B4 不採用確定として削除、B2/B5 再検討予定として残留）。 |
| Ver.3.3 | 2026-04-13 | **リリースカレンダー規則の適用・Ver.3.x/4.x 計画整合**（統合ファイルでの改訂）。§5.0 リリースカレンダー規則の適用表を新設（Ver.3.1〜3.9 種別一覧）。§5.3 Ver.3.2 をブログ機能拡充 → バグ修正（偶数バージョン準拠）に変更。§5.4 Ver.3.3 をバグ修正 → ブログ機能拡充（奇数バージョン準拠）に変更。§5.10 Ver.3.9 バグゼロ達成バージョンを新設。§6.0 リリースカレンダー規則の適用表を新設（Ver.4.0〜4.9 種別一覧）。§6.2-6.8 を新カレンダー規則に基づき §6.2-6.10（Ver.4.1-4.9）に再構成（Ver.4.1 に管理機能強化、Ver.4.2 にバグ修正を移動、Ver.4.7 新設プレースホルダー、Ver.4.8 バグ修正、Ver.4.9 バグゼロ達成追加）。 |
| Ver.3.4 | 2026-04-13 | **Portable Text 導入・Ver.3.1 X.1 破壊的変更に再分類**（統合ファイルでの改訂）。§5.0 カレンダー規則適用表の Ver.3.1 を「奇数扱い」→「X.1 破壊的変更」に変更。§5.2 Ver.3.1 を「ブログ基盤 + Portable Text 導入（X.1 破壊的変更）」に変更: PT 導入・`body` フィールド新設・旧形式自動マイグレーションの破壊的変更エントリを追加。§5.6 Ver.3.5 に旧形式全廃（廃止ポリシー適用）エントリを追加（`renderBlocksToHtml()`/`renderMarkdownToHtml()`/`markdown.ts` 削除・`content`/`format`/`blocks` フィールド廃止）。 |
| Ver.3.5 | 2026-04-15 | **Ver.3.6〜3.9 バグ修正実装記録**（統合ファイルでの改訂）。§5.7 Ver.3.6 精査50件結果（全件）を追加（致命的1・重大11・中程度21・軽微17・先送り14件）。§5.8 Ver.3.7 実装済に更新。§5.9 Ver.3.8 精査53件結果（全件）を追加（重大1・中程度7・仮陽性多数）。§5.10 Ver.3.9 精査50件結果（全件）を追加。現行バージョンを Ver.3.8-56 に更新。 |
| Ver.3.6 | 2026-04-15 | **Ver.3.9-57 バグゼロ達成・文書バージョン更新**（統合ファイルでの改訂）。§5.10 Ver.3.9 の状態を「計画」→「実装済」に更新・精査50件結果（全件）を最終確認。3. リリース履歴に Ver.3.9 エントリを追加。現行バージョンを Ver.3.9-57 に更新。 |
...
## ルート CLAUDE.md（統合リポジトリ開発規約）

> ルート `CLAUDE.md` は Adlaire Group 統合リポジトリの共通規約文書。変更は全プロジェクトに影響するため、ここで管理する。

| 日付 | 変更内容 |
|------|---------|
| 2026-04-06 | 初版（統合リポジトリ開発規約として新設） |
| 2026-04-13 | **リリースカレンダー規則を新設**。§「リリース計画ポリシー」に「リリースカレンダー規則」サブセクションを追加。Ver.X.0（破壊的変更）/ Ver.X.1（破壊的変更または改良）/ 偶数（バグ修正 50〜100件）/ 奇数（新機能15件・機能改良20〜30件）/ Ver.X.9（バグゼロ達成）の 10 バージョンサイクルを規定。Adlaire Static CMS Ver.3.1 より適用・全プロジェクトに例外なく適用。 |
