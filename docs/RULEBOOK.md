...
| 15 | コード品質 | `listPages()` にページインデックスキャッシュ導入 | 実装済 |

### 9.2 Ver.1.8 — 機能拡張・フロントエンド強化

| # | カテゴリ | 改良点 | 状態 |
|---|---------|--------|:----:|
| 16 | 新機能 | ページ並び順のドラッグ&ドロップ管理 | 実装済 |
| 17 | 新機能 | 管理 UI ダークモード対応 | 実装済 |
| 18 | 新機能 | ページプレビュー機能（下書きを公開レイアウトで確認） | 実装済 |
| 19 | 新機能 | サイドバー（subside）もブロックエディタで編集 | 実装済 |
| 20 | 新機能 | 静的生成で Markdown をサーバーサイド HTML 変換 | 実装済 |
| 21 | エディタ | ブロックのコピー＆ペースト（Ctrl+C/V） | 実装済 |
| 22 | エディタ | Undo/Redo 機能（Ctrl+Z/Y） | 実装済 |
| 23 | エディタ | ブロックのドラッグ&ドロップ並び替え | 実装済 |
| 24 | エディタ | heading ブロックのレベル変更 UI（h1/h2/h3 切替） | 実装済 |
| 25 | エディタ | list ブロックの順序/非順序切替 UI | 実装済 |
| 26 | フロント | 公開ページから管理用 JS を除外 | 実装済 |
| 27 | フロント | 静的サイト用軽量 CSS（エディタ CSS 除外） | 実装済 |
| 28 | データ | エクスポートにリビジョンを含むオプション | 実装済 |
