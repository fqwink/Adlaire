...
- `rulebookdocs/` — ルールブックドキュメントフォルダ
  - `rulebookdocs/PORTAL_SYSTEM_RULEBOOK.md` — ポータルシステム仕様（仕様の正）
  - `rulebookdocs/REVISION_HISTORY.md` — 改訂履歴
  - ※ `rulebookdocs/` 内に README.md は作成しない
  - ※ リリース計画はルート `rulebookdocs/RELEASE_PLAN_RULEBOOK.md`（Part V）で統合管理
- `docs/` — ドキュメントフォルダ
  - `docs/CHANGES.md` — 変更履歴

## 技術規約

- **HTML5 + JavaScript（バニラ）のみ**
- サーバーサイド処理なし（サーバーレス）
- 外部ライブラリ・CDN 依存禁止（セルフコンテインド原則）
- **`npm:` プレフィックスのインポート全面禁止**（Adlaire 共通規約準拠）
- データストア: LocalStorage（ブラウザ内）+ `data.js`（静的設定ファイル）
...
- Adlaire 統合リポジトリ（`fqwink/Adlaire`）の `Adlaire Portal System/` ディレクトリで管理する。
- 旧リポジトリ（`fqwink/Adlaire_Portal_System`）は廃止済み。
