...
> **注記**: §5.1-5.2 の必須アーキテクチャはライフサイクルシステムの目標設計を示す。
> 現行 MVP（`CHARTER.md` §8）では、セットアップツールを関数ベース（`detect_*()`, `validate_*()`, `install_*()`, `render_*()`, `security_*()`）で `bundle-installer.php` 単一ファイルに実装している。
> 将来の Update 機能拡張時に、本セクションのクラスベース設計への移行を検討する。
...
| `API_RULEBOOK.md` | API・データ仕様 |
| `DIRECTION_RULEBOOK.md` | 製品方向性 |
| `RELEASE_PLAN_RULEBOOK.md` | リリース計画・リリース履歴 |
