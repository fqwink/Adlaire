...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | ロールバック機能（デプロイ履歴保持・過去バージョン復元） | 計画 |
| 2 | 追加機能 | ログ SSE ストリーミング（リアルタイムログ配信） | 計画 |
| 3 | 追加機能 | Worker ヘルスチェック & 自動再起動（指数バックオフ） | 計画 |
| 4 | 追加機能 | プライベートリポジトリ対応（PAT / SSH 鍵認証・暗号化保存） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | ビルドステップ（デプロイ前の任意コマンド実行） | 計画 |
| 2 | 追加機能 | ログ永続化（ファイルシステム保存・日次ローテーション・30日保持） | 計画 |
| 3 | 機能改良 | 環境変数の AES-256-GCM 暗号化（旧平文形式を廃止・マイグレーション） | 計画 |
| 4 | 追加機能 | デプロイ通知 Webhook（HMAC-SHA256 署名対応） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 機能改良 | edge 自動回復（バックオフ付き再接続・設定再同期・自動再有効化） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | 管理ダッシュボード Web UI（プロジェクト管理・ログビューア・クラスタ状態・デプロイ操作） | 計画 |
| 2 | 追加機能 | ダッシュボード認証（セッショントークン・24時間有効期限） | 計画 |
