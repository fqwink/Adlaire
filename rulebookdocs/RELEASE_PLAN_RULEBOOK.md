...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | インストールスクリプト（`install.sh`・冪等セットアップ・自動シークレット生成） | 計画 |
| 2 | 追加機能 | systemd サービス化（ユニットファイルテンプレート・セキュリティ強化） | 計画 |
| 3 | 追加機能 | セルフアップデート（ゼロダウンタイム切替・自動ロールバック・CLI/API） | 計画 |
| 4 | 追加機能 | GitHub Actions CI/CD テンプレート（プラットフォーム自動デプロイ・Webhook ワークフロー） | 計画 |
| 5 | 追加機能 | バックアップ・リストア CLI（`deploy platform backup/restore`） | 計画 |
| 6 | 追加機能 | マルチVPS プロビジョニングスクリプト（`provision.sh`） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | 監査ログ（操作記録・KV永続化・最大10,000件） | 計画 |
| 2 | 機能改良 | デプロイパイプライン並列化（max_parallel_deploys、セマフォ制御） | 計画 |
| 3 | 追加機能 | モノレポ対応（root_dir フィールド・エントリポイント・ビルドCWD調整） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | 静的サイトホスティング（Worker不要・SPA対応・ディレクトリトラバーサル防止） | 計画 |
| 2 | 追加機能 | メトリクス収集（リクエスト数・エラー率・P99レスポンスタイム・1分粒度・60分保持） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | Blue-Green デプロイ（裏側起動・ヘルスチェック通過後切替・失敗時自動ロールバック） | 計画 |
| 2 | 追加機能 | カナリアデプロイ（重み付きルーティング・段階的トラフィック移行・昇格/中止操作） | 計画 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | デプロイスケジューリング（cron式・1回限り指定・60秒チェック間隔） | 計画 |
| 2 | 追加機能 | デプロイプレビュー URL（ブランチ単位・TTL自動削除・ポート自動割当） | 計画 |
...
**Phase 0 完了**（仕様策定済み・実装未着手）

### VI-2. リリース計画

#### Ver.1.0-1〜6 — Phase 1〜6: 基盤実装
...
| Phase | # | 種別 | 概要 | 状態 |
|:-----:|:-:|------|------|:----:|
| 1 | 1 | 追加機能 | デザイントークン（`tokens.css`） | 計画 |
| 1 | 2 | 追加機能 | リセット・ベース CSS（`reset.css`） | 計画 |
| 1 | 3 | 追加機能 | タイポグラフィ（`typography.css`） | 計画 |
| 2 | 4 | 追加機能 | レイアウトユーティリティ（`layout.css`） | 計画 |
| 2 | 5 | 追加機能 | Button コンポーネント | 計画 |
| 2 | 6 | 追加機能 | Form コンポーネント | 計画 |
| 2 | 7 | 追加機能 | Card コンポーネント | 計画 |
| 3 | 8 | 追加機能 | Badge コンポーネント | 計画 |
| 3 | 9 | 追加機能 | Table コンポーネント | 計画 |
| 3 | 10 | 追加機能 | Alert コンポーネント | 計画 |
| 3 | 11 | 追加機能 | Modal コンポーネント | 計画 |
| 4 | 12 | 追加機能 | ユーティリティクラス（`utilities.css`） | 計画 |
| 5 | 13 | 追加機能 | ダークモード対応 | 計画 |
| 6 | 14 | 追加機能 | ビルドスクリプト（`build.ts`）+ `dist/` 生成 + `Distribution/` 配布 | 計画 |
...
#### Ver.1.0-1 — Phase 1〜8: コア実装

- **種別**: 追加機能（初版）
- **対象仕様**: FRAMEWORK_RULEBOOK.md Phase 1〜8

##### 含まれる機能

| Phase | # | 種別 | 概要 | 状態 |
|:-----:|:-:|------|------|:----:|
| 1 | 1 | 追加機能 | コア HTTP サーバー（`Deno.serve` ラッパー・リクエスト/レスポンス抽象化） | 計画 |
| 2 | 2 | 追加機能 | ファイルベースルーティング（`routes/` 探索・URL マッチング・動的パラメータ） | 計画 |
| 3 | 3 | 追加機能 | 型安全システム（`defineHandler` / `Context<Params>` / パラメータ型推論） | 計画 |
| 4 | 4 | 追加機能 | ミドルウェア（`_middleware.ts` チェーン・`defineMiddleware` / 状態型付け） | 計画 |
| 5 | 5 | 追加機能 | 静的ファイル配信（`static/` ディレクトリ対応） | 計画 |
| 6 | 6 | 追加機能 | CLI ツール（`adlaire new` / `adlaire dev` / `adlaire routes`） | 計画 |
| 7 | 7 | 追加機能 | Adlaire Style 統合（自動 CSS 注入） | 計画 |
| 8 | 8 | 追加機能 | Adlaire Deploy 統合（デプロイ設定生成・環境変数連携） | 計画 |

#### Ver.2.0-（未定） — Phase 9〜10: UI・Islands（保留）

- **種別**: 追加機能
- **状態**: 保留（UIレイヤー・Islands アーキテクチャ確定後に計画策定）

| Phase | 概要 | 状態 |
|:-----:|------|:----:|
| 9 | UI レンダリング層 | 保留 |
| 10 | Islands アーキテクチャ | 保留 |
