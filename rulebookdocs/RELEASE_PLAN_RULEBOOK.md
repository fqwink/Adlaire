...
> 以下は追加機能候補であり、配分先バージョンは未定。
> 各ルールブックに仕様を策定後、リリースバージョンに配分する。

| # | カテゴリ | 機能概要 | 状態 |
|---|---------|---------|:----:|
| A1 | テーマ | テーマ設定 UI の強化（カラー/レイアウト設定、プレビュー） | 未定 |
| A2 | エディタ | コンテンツタイプ拡張（テーブル、アコーディオン、埋め込み等） | 未定 |
| A3 | メディア | 画像/ファイル管理の改善（メディアライブラリ、アップロード、サムネイル） | 未定 |
| A4 | データ | エクスポート/インポートの強化（選択的インポート、差分マージ） | 未定 |
| A5 | 静的生成 | 静的生成時の差分ビルド強化（アセット差分、変更検出の精度向上） | 未定 |
| A6 | 検索 | 検索と sitemap の運用強化（全文検索インデックス、sitemap 自動更新） | 未定 |

---

## 7. 優先度低・不採用項目

以下は現時点で採用しない。DIRECTION_RULEBOOK.md §5.7「コアは小さく保つ」原則に基づく。
将来の需要に応じて再検討する。

| 項目 | 不採用理由 |
|------|-----------|
| 複雑なプラグイン依存機構 | 現行 hooks 配列で十分。プラグイン間依存は複雑性を著しく増大させる |
| マルチユーザー高度権限管理 | 単一管理者モデルが設計前提。共有サーバー制約に反する |
| SaaS 的なチームコラボ機能 | フラットファイル CMS の設計範囲を逸脱。DB 前提の機能 |
| 重いビジュアルページビルダー化 | ブロックエディタの軽量性を損なう。EDITOR_RULEBOOK の原則に反する |
| 多数の外部サービス連携 | 依存を最小化する方針に反する。必要なら個別プラグインで対応 |
