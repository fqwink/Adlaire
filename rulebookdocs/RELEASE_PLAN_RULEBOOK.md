...
| **Adlaire Framework** | Ver.1.0-1 | 1 | Phase 1 実装済み |
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 6 | 追加機能 | ミドルウェアチェーン（`server.use()` / 状態型付け） | §6.1 | 計画 |
| 7 | 追加機能 | CORS ミドルウェア（`cors()` / `CorsOptions` / プリフライト対応） | §8.2 | 計画 |
| 8 | 追加機能 | CLI: `adlaire-fw new`（プロジェクトテンプレート生成） | 仕様未策定 | 計画 |
| 9 | 追加機能 | CLI: `adlaire-fw dev`（開発サーバー起動） | 仕様未策定 | 計画 |
| 10 | 追加機能 | CLI: `adlaire-fw check`（型検証） | 仕様未策定 | 計画 |
| 11 | 追加機能 | CLI: `adlaire-fw routes`（ルート一覧） | 仕様未策定 | 計画 |
