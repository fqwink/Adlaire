...
| **Adlaire Framework** | 未リリース | — | Phase 0 完了・基本実装計画中 |
...
- **種別**: Phase 0
- **状態**: 策定済み（FRAMEWORK_RULEBOOK.md Ver.1.19）
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 1 | 追加機能 | HTTP サーバー起動（`Deno.serve` ベース） | §1.4 / §10.1 | 計画 |
| 2 | 追加機能 | ファイルベースルーティング（`routes/` 探索・URL マッチング・動的パラメータ） | §5 | 計画 |
| 3 | 追加機能 | 型安全システム（`defineHandler` / `Context<Params>` / ルートパラメータ自動型付け） | §6 / §7 | 計画 |
| 4 | 追加機能 | レスポンスヘルパー（`ctx.json()` / `ctx.html()` / エラーレスポンス） | §7.2 / §7.3 | 計画 |
| 5 | 追加機能 | 静的ファイル配信（`static/` ディレクトリ対応） | §9 | 計画 |
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 6 | 追加機能 | ミドルウェア（`_middleware.ts` チェーン・`defineMiddleware` / 状態型付け） | §8 | 計画 |
| 7 | 追加機能 | CLI: `adlaire new`（プロジェクトテンプレート生成） | §11 | 計画 |
| 8 | 追加機能 | CLI: `adlaire dev`（開発サーバー・ファイル監視リロード） | §11 | 計画 |
| 9 | 追加機能 | CLI: `adlaire check` / `adlaire routes`（型検証・ルート一覧） | §11 | 計画 |

---

#### Ver.1.2-3 — Phase 3: デプロイ対応
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 10 | 追加機能 | デュアルデプロイ対応（Deno Deploy / Adlaire Deploy 自動判定） | §10.5 / §10.6 | 計画 |
| 11 | 追加機能 | CLI: `adlaire build --target=deno`（本番ビルド生成） | §11 | 計画 |
| 12 | 追加機能 | CLI: `adlaire build --target=js`（共用サーバ向け JS 出力） | §10.3 / §11 | 計画 |
