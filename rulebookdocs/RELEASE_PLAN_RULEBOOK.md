...
> バージョン配分が指定されていない全項目の一覧。
> §4/§5 でバージョンに配分済みの項目はここには含まない。
> 仕様策定・優先度決定後にリリースバージョンへ配分する。
> 配分が決まった項目は本セクションから削除し、該当バージョンのセクションに移動すること。

### 6.1 追加機能候補

| # | カテゴリ | 機能概要 | 優先度 | 状態 |
|---|---------|---------|:------:|:----:|
| A1 | テーマ | テーマ設定 UI の強化（カラー/レイアウト設定、プレビュー） | 通常 | 未定 |
| A2 | エディタ | コンテンツタイプ拡張（テーブル、アコーディオン、埋め込み等） | 通常 | 未定 |
| A3 | メディア | 画像/ファイル管理の改善（メディアライブラリ、アップロード、サムネイル） | 通常 | 未定 |
| A4 | データ | エクスポート/インポートの強化（選択的インポート、差分マージ） | 通常 | 未定 |
| A5 | 静的生成 | 静的生成時の差分ビルド強化（アセット差分、変更検出の精度向上） | 通常 | 未定 |
| A6 | 検索 | 検索と sitemap の運用強化（全文検索インデックス、sitemap 自動更新） | 通常 | 未定 |

### 6.2 優先度低の採用項目

> 採用するが優先度は低い。コア機能の安定後に着手する。

| # | カテゴリ | 機能概要 | 優先度 | 状態 |
|---|---------|---------|:------:|:----:|
| B1 | プラグイン | プラグイン依存機構（依存解決、ロード順管理） | 低 | 未定 |
| B2 | 管理 | マルチユーザー権限管理（ロール、アクセス制御） | 低 | 未定 |
| B3 | コラボ | チームコラボ機能（編集ロック、変更通知） | 低 | 未定 |
| B4 | エディタ | ビジュアルページビルダー（レイアウト編集、セクション管理） | 低 | 未定 |
| B5 | 連携 | 外部サービス連携（Webhook、CDN 連携等） | 低 | 未定 |

### 6.2 運用ルール

- 新しい機能候補が発生した場合、まず本セクションに追加する。
- バージョン配分が決定したら、該当バージョンのセクション（§4 または §5）に移動する。
- 不採用と判断した場合は §7（優先度低・不採用項目）に移動する。
