...
| **Adlaire Framework** | Ver.1.5-10 | 2026-04-12 | 型付け絶対原則（§0.6追加・defineHandler追加） 実装済み |
...
**Ver.1.5-10**（型付け絶対原則。§0.6追加・defineHandler追加 実装済み）
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 追加機能 | `defineHandler<P, B, Q, S>()` ファクトリ関数を `Core/router.ts`（`@adlaire/fw/router`）に追加 | 実装済 |
| 2 | 設計原則追加 | §0.6 追加（絶対原則）: 型付け絶対原則。インラインラムダのルート登録メソッドへの直接渡しを禁止 | 実装済 |
| 3 | ドキュメント更新 | §3.2 / §3.3 に `defineHandler` インポート例・エクスポート表を追加 | 実装済 |
| 4 | ドキュメント更新 | §7.6 を `defineHandler<P>()` 使用例に更新。インラインラムダ禁止を明示 | 実装済 |
...
| # | 種別 | 概要 | 状態 |
|:-:|------|------|:----:|
| 1 | 機能廃止 | `ExtractRouteParams<Path>` / `ParamKeys<Path>` 型を削除 | 実装済 |
| 2 | 機能廃止 | `TypedHandler<Path>` 型を削除 | 実装済 |
| 3 | 機能廃止 | `Router` / `RouteGroup` 全メソッドから `<Path extends string>` オーバーロードを削除 | 実装済 |
| 4 | 設計原則追加 | §0.2 強化: 型安全はフレームワークが事前決定する原則を明文化 | 実装済 |
| 5 | 設計原則追加 | §0.5 追加（絶対原則）: パスリテラル型自動推論を全域で禁止 | 実装済 |
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 1 | 追加機能 | ボディサイズ制限ミドルウェア（`bodyLimit()`） | §8.7 | 実装済 |
| 2 | 追加機能 | リクエスト ID ミドルウェア（`requestId()`） | §8.8 | 実装済 |
| 3 | 追加機能 | タイムアウトミドルウェア（`timeout()`） | §8.9 | 実装済 |
| 4 | 追加機能 | セキュリティヘッダーミドルウェア（`secureHeaders()`） | §8.10 | 実装済 |
| 5 | 追加機能 | クエリ文字列スキーマパース（`parseQuery()` / `QuerySchema` / `QueryResult<S>`） | §5.7 / §9.5 | 実装済 |
| 6 | 追加機能 | パスパラメータ型変換（`parseParam()`） | §9.6 | 実装済 |
| 7 | 追加機能 | CLI: `adlaire-fw routes`（ルート一覧） | §12.3 | 実装済 |
| 8 | 追加機能 | CLI: `adlaire-fw dev`（開発サーバー起動） | §12.4 | 実装済 |
| 9 | 追加機能 | CLI: `adlaire-fw new`（プロジェクトテンプレート生成） | §12.5 | 実装済 |
| 10 | 追加機能 | CLI: `adlaire-fw check`（型検証） | §12.6 | 実装済 |
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 1 | 追加機能 | `TypedHandler<Path>`: パスリテラル型からパラメータ型を推論するハンドラー型エイリアス | §5.10 | 実装済 |
| 2 | 追加機能 | `Simplify<T>`: mapped type をフラット展開するユーティリティ型 | §5.11 | 実装済 |
| 3 | 追加機能 | `StrictQueryResult<S>`: QueryResult から undefined を除去するユーティリティ型 | §5.12 | 実装済 |
| 4 | 機能改良 | `EnvRule` に `"enum"` 型追加（値バリデーション + リテラル Union 型推論） | §5.6 | 実装済 |
| 5 | 機能改良 | `RouteGroup` 全ルート登録メソッドに `ExtractRouteParams<Path>` 型推論オーバーロード追加 | §7.6 | 実装済 |
...
| # | 種別 | 概要 | 仕様参照 | 状態 |
|:-:|------|------|---------|:----:|
| 1 | 追加機能 | `hsts()`: HSTS ミドルウェア（max-age / includeSubDomains / preload） | §8.13 | 実装済 |
| 2 | 追加機能 | `ipFilter()`: IP フィルタリングミドルウェア（IPv4 CIDR / 許可・拒否リスト） | §8.14 | 実装済 |
| 3 | 追加機能 | `ExtractRouteParams<Path>`: パスリテラル型からパラメータ型を自動導出 | §5.8 | 実装済 |
| 4 | 追加機能 | `InferSchema<S>` / `InferFieldType<R>`: Schema 定義から TypeScript 型を自動導出 | §5.9 | 実装済 |
| 5 | 機能改良 | Router の全ルート登録メソッドに `ExtractRouteParams<Path>` 自動推論オーバーロード追加 | §7.6 | 実装済 |
| 6 | 機能改良 | `logger()`: `LogInfo` 型 + `format?: (info: LogInfo) => string` オプション追加 | §8.3 | 実装済 |
| 7 | 機能改良 | `rateLimit()`: `RateLimitStore` インターフェース + `store?` オプション追加 | §8.4 | 実装済 |
| 8 | 機能改良 | `EnvResult<S>` 刷新: `EnvValueOf<R>` ヘルパー型ベース / 任意フィールドを `T \| undefined` に変更（**破壊的変更**） | §5.6 | 実装済 |
...
| バージョン | リリース日 | 種別 | 概要 |
|-----------|-----------|------|------|
| **Ver.1.5-10** | 2026-04-12 | 追加機能・設計原則追加 | 型付け絶対原則（§0.6追加・defineHandler追加） |
| **Ver.1.4-9** | 2026-04-12 | 機能廃止・アーキテクチャ変更 | パスリテラル型自動推論廃止・禁止化（§0.5追加・ExtractRouteParams/TypedHandler削除） |
...
| **Ver.1.1-2** | 2026-04-11 | Phase 2 | ミドルウェア強化4件 + CLI4件 + 型安全強化2件 |
| **Ver.1.0-1** | 2026-04-11 | Phase 1 | コア実装 + 改良4 + 追加機能8 + バグ修正（精査55件） |
| **Ver.1.0-0** | 2026-04-11 | Phase 0 | ルールブック策定（FRAMEWORK_RULEBOOK.md Ver.1.0） |
